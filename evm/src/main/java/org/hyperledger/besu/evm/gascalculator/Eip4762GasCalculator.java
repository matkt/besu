...
public class Eip4762GasCalculator extends PragueGasCalculator {
  ...
  public long computeBaseAccessEventsCost(
    ...
    statelessCost =
        clampedAdd(statelessCost, accessWitness.accessAccountCodeHash(transaction.getSender()));
    ...
    // (tx.origin, 0, BASIC_DATA_LEAF_KEY)
    statelessCost =
        clampedAdd(statelessCost, accessWitness.writeAccountBasicData(transaction.getSender()));
  ...
  public long callOperationGasCost(
    ...
    if (!transferValue.isZero()) {
      ...
      statelessGasCost =
          clampedAdd(statelessGasCost, accessWitness.accessAccountBasicData(recipient));
      ...
      statelessGasCost =
          clampedAdd(statelessGasCost, accessWitness.writeAccountBasicData(recipient));
      if (!recipient.equals(to)) {
  ...
  public long selfDestructOperationGasCost(
    ...
    statelessGasCost =
        clampedAdd(statelessGasCost, accessWitness.accessAccountBasicData(recipientAddress));
    if (!recipientAddress.equals(originatorAddress)) {
      statelessGasCost =
          clampedAdd(statelessGasCost, accessWitness.accessAccountBasicData(originatorAddress));
    }
    // (sender, 0, BASIC_DATA_LEAF_KEY)
    // (recipient, 0, BASIC_DATA_LEAF_KEY)
    statelessGasCost =
        clampedAdd(statelessGasCost, accessWitness.writeAccountBasicData(recipientAddress));
    if (!recipientAddress.equals(originatorAddress)) {
      statelessGasCost =
          clampedAdd(statelessGasCost, accessWitness.writeAccountBasicData(originatorAddress));
    }
  ...
  public long codeCopyOperationGasCost(
    ...
    if (!frame.wasCreatedInTransaction(frame.getContractAddress())) {
      ...
      statelessGasCost =
          clampedAdd(
              statelessGasCost,
              accessWitness.touchCodeChunks(
                  frame.getContractAddress(), codeOffset, readSize, codeSize));
    }
    return clampedAdd(gasCost, statelessGasCost);
  }
  ...
  public long extCodeHashOperationGasCost(
    ...
    if (maybeAddress.isPresent()) {
      final Address address = maybeAddress.get();
      return frame.getAccessWitness().accessAccountCodeHash(address);
    }
    return 0L;
  }
  ...
  public long getSloadOperationGasCost(final MessageFrame frame, final UInt256 key) {
    return frame.getAccessWitness().accessAccountStorage(frame.getContractAddress(), key);
  }
  ...
  public long calculateStorageCost(
    ...
    statelessGasCost =
        clampedAdd(
            statelessGasCost, accessWitness.accessAccountStorage(frame.getContractAddress(), key));
    statelessGasCost =
        clampedAdd(
            statelessGasCost, accessWitness.writeAccountStorage(frame.getContractAddress(), key));
  ...
  public long txCreateCost(final MessageFrame frame) {
    final long statelessGasCost =
        frame.getAccessWitness().writeAccountBasicData(frame.getContractAddress());
  ...
  public long completedCreateContractGasCost(final MessageFrame frame) {
    ...
    return clampedAdd(
        statelessCost, accessWitness.writeAccountBasicData(frame.getContractAddress()));
  }
