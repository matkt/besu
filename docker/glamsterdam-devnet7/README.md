# Glamsterdam Devnet 7 — Besu + Prysm (Docker)

Run a local **Besu** execution client and **Prysm** beacon node connected to [Glamsterdam devnet 7](https://glamsterdam-devnet-7.ethpandaops.io/), an Ethereum protocol devnet for the Glamsterdam upgrade (ePBS / EIP-7732, BAL / EIP-7928).
...
- `curl`, `openssl`, `jq`
- Outbound internet access (to download genesis/config and peer with the devnet; CL `genesis.ssz` is ~70MB on this network)
...
By default, Besu runs from a **local Docker image** built from this repository (`besu:glamsterdam-devnet7-local`), not the ethPandaOps image. Prysm and network genesis/config still come from ethPandaOps.
...
./setup.sh start --besu-image ethpandaops/besu:glamsterdam-devnet-7
```
...
docker build \
  ...
  -t besu:glamsterdam-devnet7-local \
  build/docker-besu
```
...
|-----------|---------|-------------|
| `--data-dir` | `~/.glamsterdam-devnet7` | Base host path; Besu data at `<data-dir>/besu` |
| `--besu-data-dir` | `<data-dir>/besu` | Override Besu data mount only |
| `--prysm-image` | `ethpandaops/prysm-beacon-chain:glamsterdam-devnet-7` | Prysm Docker image |
| `--besu-image` | `besu:glamsterdam-devnet7-local` | Besu Docker image (local branch build) |
...
| `--bootnodes` | Glamsterdam devnet 7 list | Override EL `--bootnodes` |
...
| `BESU_BOOTNODES` | Glamsterdam devnet 7 list | EL bootnodes (comma-separated) |
...
./setup.sh start --data-dir /var/lib/glamsterdam7

# Public node with explicit P2P host
P2P_HOST=206.189.200.7 ./setup.sh start --data-dir /var/lib/glamsterdam7
...
./setup.sh start \
  --data-dir /tmp/glamsterdam7 \
  --prysm-image ethpandaops/prysm-beacon-chain:glamsterdam-devnet-7 \
  --besu-image ethpandaops/besu:glamsterdam-devnet-7
...
| Field | Value | Source |
|-------|-------|--------|
| Chain ID | `7082904758` | [EL genesis](https://config.glamsterdam-devnet-7.ethpandaops.io/el/besu.json) |
| Deposit contract | `0x00000000219ab540356cBB839Cbe05303d7705Fa` | [CL config](https://config.glamsterdam-devnet-7.ethpandaops.io/cl/config.yaml) |
| Deposit contract block | `0` | ethPandaOps config API |
| EL genesis | `besu.json` | `config.glamsterdam-devnet-7.ethpandaops.io/el/besu.json` |
...
| CL bootnodes | Fetched at startup (5 CL ENRs) → `prysm-bootnodes.yaml` | [Node inventory API](https://config.glamsterdam-devnet-7.ethpandaops.io/api/v1/nodes/inventory); Prysm needs one ENR per YAML list entry (comma-separated CLI values are invalid) |
| Static EL peers | Optional (`--use-static-nodes`) | Inventory API → `static-nodes.json` |
| Checkpoint sync | `https://checkpoint-sync.glamsterdam-devnet-7.ethpandaops.io` | ethPandaOps bootnode |

Fork highlights (from genesis): merge at block 0, **Amsterdam** fork at timestamp `1784044992` (Glamsterdam EL features).

## Architecture

```
┌─────────────────────────────────────────────────────────┐
│  Docker network: glamsterdam-devnet7                      │
│                                                           │
│  ┌──────────────┐  Engine API (8551)   ┌──────────────┐  │
│  │    Besu      │◄──── JWT auth ──────►│    Prysm     │  │
│  │  (EL/sync)   │                      │  (CL/sync)   │  │
...
└─────────┼──────────────────────────────────────┼──────────┘
          │                                      │
          ▼                                      ▼
   Glamsterdam devnet 7 peers (ethPandaOps)
```
...
./setup.sh restart-prysm --data-dir /var/lib/glamsterdam7
./setup.sh resync-prysm --confirm --data-dir /var/lib/glamsterdam7
```
...
cat ~/.glamsterdam-devnet7/config/static-nodes.json | jq 'length'
...
1. **Local Besu vs devnet images** — By default, Besu is built from the **current repo branch** (`besu:glamsterdam-devnet7-local`). Prysm still uses `ethpandaops/prysm-beacon-chain:glamsterdam-devnet-7`. Network genesis/config always come from ethPandaOps. Use `--besu-image ethpandaops/besu:glamsterdam-devnet-7` if you prefer the published devnet image.

2. **Devnet can reset** — ethPandaOps may redeploy devnet 7. If sync stalls after an upgrade, run `./setup.sh stop`, clear Besu/Prysm data dirs if needed, then `./setup.sh start --refresh-config`. To retry Prysm checkpoint sync without restarting Besu, use `./setup.sh resync-prysm --confirm`.
...
docker inspect glamsterdam7-besu --format '{{range .Mounts}}{{if eq .Destination "/execution-auth.jwt"}}{{.Source}}{{end}}{{end}}'
...
- [Glamsterdam devnet 7 dashboard](https://glamsterdam-devnet-7.ethpandaops.io/)
- [Network spec (HackMD)](https://notes.ethereum.org/@ethpandaops/glamsterdam-devnet-7)
