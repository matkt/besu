...
public class BonsaiWorldStateKeyValueStorageCacheTest {
  ...
  void commitWritesVersionedAccountAndStorageTrieNodes() throws Exception {
    ...
    assertThat(head.getAccountStateTrieNode(accountLocation, accountNodeHash))
  ...
  void missingTrieNodeReadDoesNotPopulateCache() throws Exception {
    ...
    assertThat(
            head.getAccountStateTrieNode(location, Bytes32.wrap(Hash.hash(Bytes.of(1)).getBytes())))
        .isEmpty();
    assertThat(head.isCached(TRIE_BRANCH_STORAGE, location)).isFalse();
  }
  ...
  void storageTrieNodesAreCachedPerAccount() throws Exception {
    ...
    assertThat(
            head.getAccountStorageTrieNode(
                accountA, location, Bytes32.wrap(Hash.hash(nodeA).getBytes())))
        .contains(nodeA);
    assertThat(
            head.getAccountStorageTrieNode(
                accountB, location, Bytes32.wrap(Hash.hash(nodeB).getBytes())))
        .contains(nodeB);
