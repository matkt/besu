...
import org.hyperledger.besu.ethereum.trie.pathbased.bonsai.archive.BonsaiArchiveReadFlatDbStrategyProvider;
...
public class BonsaiWorldStateKeyValueStorageCacheTest {
  ...
  private void newFullHead() throws Exception {
    disposeHead();
    head =
        new BonsaiWorldStateKeyValueStorage(
            new InMemoryKeyValueStorageProvider(),
    ...
    head.upgradeToFullFlatDbMode();
    assertThat(head.getFlatDbMode()).isEqualTo(FlatDbMode.FULL);
    assertThat(head.getFlatDbStrategy()).isInstanceOf(BonsaiFullFlatDbStrategy.class);
  }

  private void newPartialHead() throws Exception {
    disposeHead();
    head =
        new BonsaiWorldStateKeyValueStorage(
            new InMemoryKeyValueStorageProvider(),
            new NoOpMetricsSystem(),
            dataConfigBuilder(true)
                .extraStorageConfiguration(
                    ImmutableExtraStorageConfiguration.builder()
                        .unstable(
                            ImmutableExtraStorageConfiguration.Unstable.builder()
                        ...
                        .build())
                .build());
    assertThat(head.getFlatDbMode()).isEqualTo(FlatDbMode.PARTIAL);
    assertThat(head.getFlatDbStrategy()).isInstanceOf(BonsaiPartialFlatDbStrategy.class);
  }

  private void newArchiveReadHead() throws Exception {
    disposeHead();
    final var config =
    ...
    final var underlying =
        new BonsaiWorldStateKeyValueStorage(new InMemoryKeyValueStorageProvider(), metrics, config);
    final var archiveProvider = new BonsaiArchiveReadFlatDbStrategyProvider(metrics, config);
    ...
    assertThat(head.getFlatDbStrategy()).isInstanceOf(BonsaiArchiveFlatDbStrategy.class);
  }
  ...
  @Test
  void getMultipleKeysInFullMode_fetchesFromStorageAndWarmsCache() throws Exception {
    newFullHead();
    final Hash present = Hash.hash(Bytes.of(1));
    final Hash missing = Hash.hash(Bytes.of(2));
    ...
    final List<Optional<Bytes>> values =
        head.getMultipleKeys(
            ACCOUNT_INFO_STATE,
            List.of(present.getBytes().toArray(), missing.getBytes().toArray()));

    // Contrast with partial/archive: real fetch + Optional.empty() for absent (not null).
  ...
  @Test
  void getMultipleKeysInFullMode_returnsCacheHitsAndFetchesMisses() throws Exception {
    newFullHead();
    final Hash cached = Hash.hash(Bytes.of(1));
    final Hash inStorageOnly = Hash.hash(Bytes.of(2));
    final Bytes cachedValue = Bytes.of(11);
    final Bytes storageValue = Bytes.of(22);
    commitAccount(cached, cachedValue);
    commitAccount(inStorageOnly, storageValue);
    head.getCacheManager().clear(ACCOUNT_INFO_STATE);
    head.getCacheManager()
        .putInCache(ACCOUNT_INFO_STATE, cached.getBytes(), cachedValue, head.getCurrentVersion());

    final List<Optional<Bytes>> values =
        head.getMultipleKeys(
            ACCOUNT_INFO_STATE,
            List.of(inStorageOnly.getBytes().toArray(), cached.getBytes().toArray()));

    // Contrast with partial/archive: storage miss is fetched (not left null / uncached).
    assertThat(values).containsExactly(Optional.of(storageValue), Optional.of(cachedValue));
    assertThat(head.isCached(ACCOUNT_INFO_STATE, cached.getBytes())).isTrue();
    assertThat(head.isCached(ACCOUNT_INFO_STATE, inStorageOnly.getBytes())).isTrue();
  }

  @Test
  void getMultipleKeysInPartialMode_storagePresentButMultigetIsNoOp() throws Exception {
    newPartialHead();
    final Hash account = Hash.hash(Bytes.of(1));
    final Bytes value = Bytes.of(1, 2, 3);
    ...
    // null = unresolved (strategy no-op), not Optional.empty() (= key absent)
    assertThat(values).hasSize(1);
    assertThat(values.get(0)).isNull();
    assertThat(head.isCached(ACCOUNT_INFO_STATE, account.getBytes())).isFalse();
    // Scalar read still finds the committed flat value.
    assertThat(head.getAccount(account)).contains(value);
  }

  @Test
  void getMultipleKeysInPartialMode_returnsCacheHitsOnly_missesStayNullAndUncached()
      throws Exception {
    newPartialHead();
    final Hash cached = Hash.hash(Bytes.of(1));
    final Hash inStorageOnly = Hash.hash(Bytes.of(2));
    ...
    assertThat(values).hasSize(2);
    assertThat(values.get(0)).isNull();
    assertThat(values.get(1)).contains(cachedValue);
    assertThat(head.isCached(ACCOUNT_INFO_STATE, cached.getBytes())).isTrue();
    assertThat(head.isCached(ACCOUNT_INFO_STATE, inStorageOnly.getBytes())).isFalse();
  }

  @Test
  void getMultipleKeysInArchiveMode_storagePresentButMultigetIsNoOp() throws Exception {
    newArchiveReadHead();
    final Hash account = Hash.hash(Bytes.of(1));
    final Bytes value = Bytes.of(1, 2, 3);
    commitAccount(account, value);
  ...
  @Test
  void getMultipleKeysInArchiveMode_returnsCacheHitsOnly_missesStayNullAndUncached()
      throws Exception {
    newArchiveReadHead();
    final Hash cached = Hash.hash(Bytes.of(1));
    final Hash inStorageOnly = Hash.hash(Bytes.of(2));
