...
import static org.hyperledger.besu.ethereum.trie.pathbased.bonsai.provider.WorldStateQueryParams.withStateRootAndBlockHashAndUpdateNodeHead;
...
class BonsaiWorldStateProviderTest {
  ...
  @Mock private BonsaiWorldStateCacheManager worldStateCacheManager;
  ...
  void shouldReturnEmptyWhenLoadingMoreThanMaxLayersBack() {
    ...
    assertThat(bonsaiWorldStateArchive.getWorldState(withBlockHeaderAndNoUpdateNodeHead(genesis)))
        .isEmpty();
    verify(worldStateCacheManager, never()).getWorldState(any(Hash.class));
  }
  ...
  void shouldReturnWorldStateWhenLoadingLessThanMaxLayersBack() {
    ...
    when(worldStateCacheManager.getWorldState(genesis.getHash()))
