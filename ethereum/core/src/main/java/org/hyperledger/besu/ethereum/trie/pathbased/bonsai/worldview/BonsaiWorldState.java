...
public class BonsaiWorldState extends PathBasedWorldState {
  ...
  public void persist(final BlockHeader blockHeader, final StateRootCommitter committer) {
    ...
    LOG.atDebug()
        .setMessage(
            "State root compute via StoredMerklePatriciaTrie (+ immutable node cache): baseRoot={} head={} nodes={} block={}")
  ...
  public Hash rootHash() {
    if (isStorageFrozen && accumulator.isAccumulatorStateChanged()) {
      ...
      LOG.atDebug()
          .setMessage(
              "Frozen rootHash via StoredMerklePatriciaTrie (+ immutable node cache): baseRoot={} nodes={}")
          .addArgument(baseRoot::toShortHexString)
          .addArgument(treeCache::cachedNodeCount)
          .log();
      try (TreeTraversalLock ignored = treeCache.beginTraversal(baseRoot, RootKind.STATE)) {
