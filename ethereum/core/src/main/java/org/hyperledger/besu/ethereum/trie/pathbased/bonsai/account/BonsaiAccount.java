 ...
 * mutable storage root (patched by the MPT state-root committers at commit time); the binary
 * strategy is stateless (the partitioned binary trie has no per-account storage root).
 *
 * <p>{@code BonsaiAccount} itself is format-agnostic: callers inject the strategy selected from the
 * active storage format (world state / accumulator / decode factory). {@link #getStorageRoot()} and
...
public class BonsaiAccount implements MutableAccount, AccountValue {
   ...
   *
   * <p>Only meaningful for MPT accounts. Binary accounts (carrying a {@link
   * BinaryStorageRootStrategy}) have no storage root, so calling this method is a programming error
   * and throws {@link UnsupportedOperationException} via the strategy. Callers that may handle
  ...
  /** Returns {@code true} when this account carries a storage root (MPT accounts only). */
  @Override
  public boolean hasStorageRoot() {
    return storageRootStrategy.hasStorageRoot();
  }
  ...
  public String toString() {
    return "AccountState{"
        ...
        + balance
        + ", storageRoot="
        + (storageRootStrategy.hasStorageRoot()
            ? storageRootStrategy.getStorageRoot()
            : "<binary: no storage root>")
        + ", codeHash="
        + codeHash
        + '}';
  }
