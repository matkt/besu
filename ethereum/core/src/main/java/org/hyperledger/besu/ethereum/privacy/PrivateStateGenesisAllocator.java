  ...
  public void applyGenesisToPrivateWorldState(
      ...
      genesis
          ...
          .forEach(
              (genesisAccount) -> {
                ...
                genesisAccount.getStorage().forEach((key, value) -> account.setStorageValue(new StorageSlotKey(key), value));
              });
    }
    ...
    if (isFlexiblePrivacyEnabled) {
      ...
      procyContract.setStorageValue(
          new StorageSlotKey(UInt256.ZERO), UInt256.fromBytes(Bytes32.leftPad(DEFAULT_FLEXIBLE_PRIVACY_MANAGEMENT)));
    }
