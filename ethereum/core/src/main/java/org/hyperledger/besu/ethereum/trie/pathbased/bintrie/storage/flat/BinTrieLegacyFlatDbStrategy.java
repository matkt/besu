...
/**
 * Strategy for managing BinTrie accounts in the flat database as before BinTrie was introduced.
 *
 * <p>This strategy handles accounts, slots, and code similar to the legacy approach, ensuring
 * consistency and compatibility with the pre-BinTrie structure.
 */
public class BinTrieLegacyFlatDbStrategy extends FlatDbStrategy {
  ...
  public BinTrieLegacyFlatDbStrategy(
  ...
  public Optional<BinTrieAccount> getFlatAccount(
    ...
    final Optional<BinTrieAccount> accountFound =
        storage
            .get(ACCOUNT_INFO_STATE, address.addressHash().toArrayUnsafe())
            .map(Bytes::wrap)
            .map(encoded -> BinTrieAccount.fromLegacyFormat(context, address, encoded, true));
  ...
  public void clearAll(final SegmentedKeyValueStorage storage) {
    // NOOP
    // we cannot clear flatdb in BinTrie as we are using directly the trie
  }
