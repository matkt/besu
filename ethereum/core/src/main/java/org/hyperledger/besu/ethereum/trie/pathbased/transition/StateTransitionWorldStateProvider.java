...
public class StateTransitionWorldStateProvider implements WorldStateArchive {
        ...
        event -> {
          if (event.getBlock().getHeader().getTimestamp() > binTrieMilestone
            ...
            PreImageDownloaderV2.generatePreloadedStateData(
                    (BonsaiWorldState) bonsaiProvider.getWorldState(), (BinTrieWorldState) binTrieProvider.getWorldState(), 5000)
  ...
  private Optional<BinTrieWorldState> getBinTrieTransitionState(
    ...
    System.out.println(
        "binTrieState.getWorldStateBlockHash().isZero() "
            + binTrieState.getWorldStateBlockHash().isZero());
    if (binTrieState.getWorldStateBlockHash().isZero()) {
      System.out.println(
          "worldStateQueryParams.shouldWorldStateUpdateHead() "
              + worldStateQueryParams.shouldWorldStateUpdateHead());
