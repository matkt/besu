...
import org.hyperledger.besu.ethereum.eth.sync.tasks.DownloadHeaderSequenceTaskV2;
...
public class DownloadHeadersStepV2
    implements Function<CheckpointRange, CompletableFuture<CheckpointRangeHeaders>> {
  private static final Logger LOG = LoggerFactory.getLogger(DownloadHeadersStepV2.class);
  ...
  private CompletableFuture<List<BlockHeader>> downloadHeaders(
    ...
    if (checkpointRange.hasEnd()) {
      ...
      return DownloadHeaderSequenceTaskV2.endingAtHeader(
              protocolSchedule,
              protocolContext,
              ethContext,
