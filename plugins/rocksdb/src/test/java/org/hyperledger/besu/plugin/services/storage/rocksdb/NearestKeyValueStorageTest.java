...
public class NearestKeyValueStorageTest {
  ...
  public void testNearestRocksdbWithInMemoryKeyValueStorage() {
    final SegmentedKeyValueStorage rockdDBKeyValueStorage =
        getRocksDBKeyValueStorage(PATRICIA_TRIE_BRANCH_STORAGE);
    ...
    IntStream.range(1, 10)
        .forEach(
            i -> {
              ...
              rocksDbTransaction.put(PATRICIA_TRIE_BRANCH_STORAGE, key, value);
              inMemoryDBTransaction.put(PATRICIA_TRIE_BRANCH_STORAGE, key, value);
              ...
              rocksDbTransaction.put(PATRICIA_TRIE_BRANCH_STORAGE, key2, value2);
              inMemoryDBTransaction.put(PATRICIA_TRIE_BRANCH_STORAGE, key2, value2);
              // different size:
              final byte[] key3 = Bytes.fromHexString("0x01011" + (10 - i)).toArrayUnsafe();
              final byte[] value3 = Bytes.fromHexString("0FFF").toArrayUnsafe();
              rocksDbTransaction.put(PATRICIA_TRIE_BRANCH_STORAGE, key3, value3);
              inMemoryDBTransaction.put(PATRICIA_TRIE_BRANCH_STORAGE, key3, value3);
              final byte[] key4 = Bytes.fromHexString("0x0" + (10 - i)).toArrayUnsafe();
              final byte[] value4 = Bytes.fromHexString("0FFF").toArrayUnsafe();
              rocksDbTransaction.put(PATRICIA_TRIE_BRANCH_STORAGE, key4, value4);
              inMemoryDBTransaction.put(PATRICIA_TRIE_BRANCH_STORAGE, key4, value4);
            });
    rocksDbTransaction.commit();
    inMemoryDBTransaction.commit();

    // compare rocksdb implementation with inmemory implementation
    rockdDBKeyValueStorage.stream(PATRICIA_TRIE_BRANCH_STORAGE)
        .forEach(
            pair -> {
              ...
              assertThat(
                      isNearestKeyValueTheSame(
                          inMemoryDBKeyValueStorage.getNearestBefore(
                              PATRICIA_TRIE_BRANCH_STORAGE, key),
              ...
              assertThat(
                      isNearestKeyValueTheSame(
                          inMemoryDBKeyValueStorage.getNearestBefore(
                              PATRICIA_TRIE_BRANCH_STORAGE, biggerKey),
                          rockdDBKeyValueStorage.getNearestBefore(
                              PATRICIA_TRIE_BRANCH_STORAGE, biggerKey)))
                  .isTrue();
              assertThat(
              ...
              assertThat(
                      isNearestKeyValueTheSame(
                          inMemoryDBKeyValueStorage.getNearestBefore(
                              PATRICIA_TRIE_BRANCH_STORAGE, smallerKey),
                          rockdDBKeyValueStorage.getNearestBefore(
                              PATRICIA_TRIE_BRANCH_STORAGE, smallerKey)))
                  .isTrue();
              assertThat(
              ...
              assertThat(
                      isNearestKeyValueTheSame(
                          inMemoryDBKeyValueStorage.getNearestBefore(
                              PATRICIA_TRIE_BRANCH_STORAGE, reversedKey),
                          rockdDBKeyValueStorage.getNearestBefore(
                              PATRICIA_TRIE_BRANCH_STORAGE, reversedKey)))
                  .isTrue();
              assertThat(
        ...
        .forEach(
            i -> {
              ...
              if (i % 2 == 0) {
                ...
                layeredDBTransaction.put(PATRICIA_TRIE_BRANCH_STORAGE, key, value);
              }
              // different common prefix, and reversed order of bytes:
              final byte[] key2 = Bytes.fromHexString("0x010" + (10 - i)).toArrayUnsafe();
              final byte[] value2 = Bytes.fromHexString("0FFF").toArrayUnsafe();
              ...
              if (i % 2 == 0) {
                ...
                layeredDBTransaction.put(PATRICIA_TRIE_BRANCH_STORAGE, key2, value2);
              }
              // different size:
              final byte[] key3 = Bytes.fromHexString("0x01011" + (10 - i)).toArrayUnsafe();
              final byte[] value3 = Bytes.fromHexString("0FFF").toArrayUnsafe();
              ...
              if (i % 2 == 0) {
                ...
                layeredDBTransaction.put(PATRICIA_TRIE_BRANCH_STORAGE, key3, value3);
              }
              final byte[] key4 = Bytes.fromHexString("0x0" + (10 - i)).toArrayUnsafe();
              final byte[] value4 = Bytes.fromHexString("0FFF").toArrayUnsafe();
              rocksDbTransaction.put(PATRICIA_TRIE_BRANCH_STORAGE, key4, value4);
              ...
              if (i % 2 == 0) {
                ...
                layeredDBTransaction.put(PATRICIA_TRIE_BRANCH_STORAGE, key4, value4);
              }
            });
    rocksDbTransaction.commit();
    inMemoryDBTransaction.commit();
